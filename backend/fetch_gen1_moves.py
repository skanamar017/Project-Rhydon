...
for url in move_urls:
    ...
    if type_ in [
        ...
        insert_lines.append(f"INSERT INTO Moves (name, type, power, accuracy, pp) VALUES ('{name}', '{type_}', {power}, {accuracy}, {pp});")

with open('gen1_moves_inserts.sql', 'w') as f:
    f.write('\n'.join(insert_lines))

print(f"Generated {len(insert_lines)} SQL insert statements for Gen 1 moves.")
